...
tab1, tab2, tab3, tab4 = st.tabs([
    "✍️ Manual Entry", "📁 Bulk CSV Upload", "📋 View & Edit Records", "🧹 Data Quality"
])
...
# ══════════════════════════════════════════════════════════════════════════════
with tab4:
    st.markdown('<div class="bz-section-title">📊 Live Data Quality Report</div>', unsafe_allow_html=True)
    try:
        ...
        if res.data:
            ...
            c2.metric("Unique Products", df['Product'].nunique() if 'Product' in df.columns else 0)
            ...
            st.dataframe(nulls, hide_index=True, use_container_width=True)
        else:
            st.info("📭 No data yet.")
    except Exception as e:
        st.error(f"Error: {e}")

                ...
                if not email or not password:
                    ...
                    try:
                        ...
                        if "email not confirmed" in err_msg or "email_not_confirmed" in err_msg:
                            st.error("📧 Email not confirmed. Please check your inbox and click the confirmation link, then try again.")
                            st.info("💡 Tip: Check your spam folder too. Or disable email confirmation in your Supabase Dashboard → Authentication → Providers → Email → toggle off 'Confirm email'.")
                            ...
                            st.warning("⏳ Too many login attempts. Please wait a few minutes and try again.")
                        else:
            ...
            if st.form_submit_button("🚀 Create Account", width='stretch'):
                if not new_email or not new_pass:
                    ...
                    try:
                        ...
                        st.success("✅ Account created! Check your inbox for a confirmation email, then switch to the Sign In tab.")
                        st.info("💡 No confirmation email? Go to Supabase Dashboard → Authentication → Providers → Email → disable **Confirm email**, then try signing in directly.")
                        st.balloons()
                    except Exception as e:
                        err = str(e).lower()
                        if "already registered" in err or "user_already_exists" in err:
                            ...
                            st.error("⚠️ Password too weak. Use at least 6 characters.")
                        else:
